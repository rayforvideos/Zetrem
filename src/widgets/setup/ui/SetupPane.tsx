  ...
  return (
    <div className="zt-scroll relative z-[3] h-full overflow-y-auto">
        ...
        <Field>
          ...
          {auth?.missing ? (
            ...
            <>
              <div className="flex flex-wrap items-center gap-3 rounded-xl bg-card px-3.5 py-2.5 text-sm">
                <span className="min-w-0 flex-1 truncate">
                  {auth.email}
                  {auth.orgName && <span className="text-muted-foreground"> · {auth.orgName}</span>}
                </span>
                <Button
                  size="sm"
                  variant="ghost"
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {loginNote && (
                <code className="font-mono text-xs text-muted-foreground">{loginNote}</code>
              )}
            </div>
          )}
        </Field>
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        {!canStart && (
          <span className="text-sm text-muted-foreground">Set your account and project first</span>
        )}
      </div>
      </div>
    </div>
