...
import { ftsQuery, linkTargets, openVaultIndex } from './vault-index'
...
const open: VaultIndex[] = []
function index(file = ':memory:'): VaultIndex {
  const idx = openVaultIndex(file)
  open.push(idx)
  return idx
}
...
describe('schema', () => {
  it('drops and recreates the tables when the version on disk differs', () => {
    const dir = mkdtempSync(join(tmpdir(), 'vault-index-'))
