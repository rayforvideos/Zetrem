...
5. **팀원들의 보고를 하나로 정리해 답한다.** 대화는 프로젝트마다 저장되므로
   다시 열면 이어서 진행할 수 있다.
6. **팀이 기다리면 어디에 있든 알린다.** 승인 요청이나 턴 끝의 질문이 생기면
   사이드바의 그 대화에 표시가 붙고, 창이 다른 창 뒤에 있으면 시스템 알림이,
   다른 대화를 보고 있으면 토스트가 뜨며, 2분 뒤 한 번 더 알린다. 답하는 순간
   전부 사라진다.
...
'언제 부를지'를 읽고 누구에게 맡길지 정한다. 팀원은 한 프로젝트 전용이거나 모든
프로젝트용이며, 팀 목록도 그 둘로 나뉜다. 자기 worktree 에서 일할지도 팀원마다
정한다.
...
| | |
|---|---|
| **라이브러리** | 프로젝트마다 두는 노트. 앱이 보관하며 프로젝트 폴더에는 아무것도 남기지 않는다. 에이전트는 모른다고 답하기 전에 여기를 찾아보고, 남기고 싶은 것이 있으면 제안 카드로 올린다. 사용자가 수락하거나 거절하며, 저절로 들어가는 것은 없다. 답변 아래 「라이브러리에」로 답변을 담을 수 있고, 입력창의 스위치로 프로젝트별로 세션에 줄지 정한다. |
| **팀원** | 앱에서 만들고, 한 프로젝트 전용이거나 모든 프로젝트용이다. 모델·도구·읽을 문서를 각각 지정하고, 원하면 자기 worktree 에서 일한다. |
...
`trash/` 폴더로 옮겨지고, 바로 삭제되지 않는다. 프로젝트의 라이브러리는 같은 데이터
디렉터리의 `library/` 안에 프로젝트 이름을 딴 SQLite 파일 하나로 저장되며, 프로젝트
폴더에는 앱의 파일이 남지 않는다. 각 라이브러리는
