...
export function runSettled<T>(plan: RunSettled<T>): Promise<T> {
  return new Promise((resolve) => {
    ...
    if (plan.mergeStderr === true) child.stderr.setEncoding('utf8')
    ...
    if (plan.mergeStderr === true) child.stderr.on('data', take)
    child.on('error', (cause: Error) => stop(plan.error(cause), false))
    child.on('exit', (code) => stop(plan.exit(code, text), false))
  })
}
