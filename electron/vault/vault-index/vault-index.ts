...
import type { NoteSource, VaultHit, VaultNoteSummary } from '@/entities/vault/model/note'
import type { IndexedNote, VaultIndex } from './vault-index.types'
...
function summaryOf(row: Row): VaultNoteSummary {
  return {
    id: row.id,
    folder: row.folder,
    title: row.title,
...
export function openVaultIndex(file: string): VaultIndex {
  ...
  const putNote = db.prepare(
    `INSERT INTO notes (id, folder, title, summary, source, tags, created_at_ms, updated_at_ms, hash)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  )
  ...
  function search(query: string, limit = DEFAULT_LIMIT): VaultHit[] {
  ...
  function recent(limit = DEFAULT_LIMIT): VaultNoteSummary[] {
    return (latest.all(limit) as Row[]).map(summaryOf)
  }

  function backlinks(title: string): VaultNoteSummary[] {
