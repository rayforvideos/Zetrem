...
const CREW = [
  ...
  },
  {
    name: 'Wren',
    face: 'flower',
    task: 'Cart totals and rounding',
    clock: '0:48',
    state: 'working' as const,
  },
  { name: 'Pace', face: 'bunny', task: 'Stock rules', clock: '2:04', state: 'reported' as const },
]
...
const MODES = ['Ask first', 'Auto-edit', 'Allow all']
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4" style={dim('talk')}>
            <div className="flex min-h-0 flex-1 flex-col gap-4 rounded-2xl border border-border bg-card p-5">
              <span className="ml-auto max-w-[78%] rounded-2xl bg-muted px-3.5 py-2 text-sm leading-snug">
                Find why checkout drops the last item
              </span>
              <span className="text-sm leading-relaxed text-muted-foreground">
                Three teammates are on it: the checkout flow, the cart totals and the stock rules. I
                will fold their reports into one answer.
              </span>
            </div>
            ...
            >
              <span className="flex-1 text-sm text-muted-foreground">Keep going</span>
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden rounded-2xl border border-border bg-card">
      <div className="flex w-[300px] flex-none flex-col gap-5 border-r border-border p-5">
        ...
        <span className="flex flex-col gap-1.5">
          <span className="text-xs text-muted-foreground">When to call them</span>
          <span className="rounded-xl border border-border px-3 py-2 text-sm">
            Reviews the front end
          </span>
          <span className="text-xs text-muted-foreground">
            The orchestrator reads this to pick who gets the job.
          </span>
        </span>
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-2 p-5">
        <span className="text-xs text-muted-foreground">Their brief</span>
        <span className="flex min-h-0 flex-1 flex-col gap-3 rounded-xl border border-border p-4 font-mono text-sm leading-relaxed text-muted-foreground">
          <span>You take the front end. Read the markup before you judge the styles.</span>
          <span>Say what you would change and why, in that order.</span>
        </span>
        <span className="text-xs text-muted-foreground">
          Standing instructions. Write it the way you would brief a person.
        </span>
