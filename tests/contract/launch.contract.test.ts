...
// On Windows npm puts a claude.cmd shim on the PATH, and Node will not spawn
// one directly, so this is where that fix is either true or not.
describe('the installed CLI can actually be launched the way the app launches it', () => {
  ...
  it('keeps an argument holding JSON intact all the way to the CLI', async () => {
    // --agents carries JSON. A shell would strip its quotes and the CLI would
    // reject the spec rather than start.
