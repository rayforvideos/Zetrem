...
const FENCE = '---'
const KNOWN = new Set(['title', 'created', 'updated', 'source', 'session', 'tags'])
...
function sourceOf(raw: string): NoteSource {
  return raw === 'agent' ? 'agent' : 'person'
}
...
export function parseNote(text: string): ParsedNote {
  ...
  const session = fields.has('session') ? unquoted(fields.get('session') ?? '') : ''
  return {
    meta: {
      title: unquoted(fields.get('title') ?? ''),
      created: unquoted(fields.get('created') ?? ''),
      updated: unquoted(fields.get('updated') ?? ''),
      source: sourceOf(unquoted(fields.get('source') ?? '')),
...
export function serializeNote(meta: NoteMeta, body: string): string {
  ...
  head.push(`source: ${meta.source}`)
  if (meta.session !== null) head.push(`session: ${scalar(meta.session)}`)
